...
class AudioProcessor {
  constructor(callSid, ws, streamSid) {
    ...
    this.energyThreshold = 100; // Minimum energy threshold
  ...
  mulawToPcmEnhanced(mulawData) {
    ...
    for (let i = 0; i < mulawData.length; i++) {
      ...
      if (Math.abs(pcm) < noiseGate) {
        pcm = Math.abs(this.lastSample) > (noiseGate + hysteresis) ? pcm * 0.1 : 0;
      }
      
      // Apply 3-point smoothing filter to reduce artifacts
      const smoothed = (pcm + this.previousSamples[0] + this.previousSamples[1]) / 3;
      ...
      const limited = this.softLimit(smoothed, 30000);
  ...
  async processRecording() {
    ...
    try {
      ...
      if (audioLengthSeconds < 0.2) {
        console.log(`${timestamp()} 🛑 Audio too short (${audioLengthSeconds.toFixed(2)}s), skipping transcription.`);
        return;
      }
      
      // For very short audio (0.2-0.5s), apply stricter voice validation
      const isShortAudio = audioLengthSeconds < 0.5;
      
      // Check maximum audio length
      if (combinedBuffer.length > this.maxAudioLength) {
        console.log(`${timestamp()} ⚠️ Audio too long, truncating to ${this.maxAudioLength} bytes.`);
      ...
      const voiceThreshold = isShortAudio ? 400 : 600;
      if (!this.hasVoiceInWav(wavBuffer, voiceThreshold)) {
        console.log(`${timestamp()} 🛑 No voice detected in wavBuffer (threshold: ${voiceThreshold}), skipping transcription.`);
        return;
      }
  ...
  hasVoiceInWav(wavBuffer, threshold = 600) {
    ...
    const peakThreshold = 800; // Lowered from 1000
    const significantThreshold = 300; // Lower threshold for detecting any meaningful audio
    ...
    for (let i = 0; i < pcmData.length; i += 2) {
      ...
      if (Math.abs(sample) > significantThreshold) {
        significantSamples++;
      }
    }
    ...
    console.log(`[VoiceCheck] RMS: ${rms.toFixed(2)}, Peak Ratio: ${peakRatio.toFixed(4)}, ` +
                `Significant Ratio: ${significantRatio.toFixed(4)} (Threshold: ${threshold})`);
    
    // For short audio, be more lenient - accept if there's any significant audio activity
    const audioLengthSeconds = pcmData.length / (8000 * 2);
    if (audioLengthSeconds < 0.5) {
      return rms > (threshold * 0.7) && significantRatio > 0.05; // More lenient for short audio
    }
    
    // Voice detected if RMS is above threshold AND there are sufficient peaks
    return rms > threshold && (peakRatio > 0.001 || significantRatio > 0.1);
  }
